...
export async function GET(request, { params }) {
  ...
  try {
    const upstream = await fetch(`${API_BASE}/downloads/${encodeURIComponent(processId)}`, {
      cache: 'no-store',
    });
    ...
    console.error('download job status error:', error);
    return Response.json(
      { success: false, message: error?.message || 'Failed to query download job status' },
      { status: 500 }
    );
  }
}
...
function normalizeJobPayload(payload) {
  ...
  if (!job.message) {
    job.message = job.downloadReady ? 'Download ready' : 'Processing download...';
  }
