// app/payment/error/page.js
// Payment error page - handles errors from ARB Payment Gateway
...
export default function PaymentErrorPage() {
  const [errorMessage, setErrorMessage] = useState('An error occurred during payment processing.');
  const searchParams = useSearchParams();

  useEffect(() => {
    // Parse error parameters from ARB gateway
    const errorCode = searchParams.get('errorCode');
    const errorText = searchParams.get('errorText');
    const paymentId = searchParams.get('paymentId');

    if (errorText) {
      setErrorMessage(decodeURIComponent(errorText));
    } else if (errorCode) {
